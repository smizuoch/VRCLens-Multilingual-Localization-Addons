...
既存4言語も[個別監査](LEGACY_TERMINOLOGY.md)の対象に追加しました。1,012件中12件の表示名を修正。
表記一致147件、根拠付き作成139件、追加確認726件です。以下の表と従来のCSVは追加22言語の集計です。
既存4言語の記録は [LegacyTranslationSources.csv](LegacyTranslationSources.csv)、
未確定一覧は [LegacyTranslationReviewQueue.csv](LegacyTranslationReviewQueue.csv)。
全26言語では6,578レコード、表記一致949件、根拠付き作成等957件、未確定4,672件です。
...
- 公式表記を画面幅に合わせて削る処理はありません。追加22言語のCSVと既存4言語の監査CSVを
  分けて保持しています。既存4言語の意図した12件の修正以外の出力は回帰検証で維持します。
